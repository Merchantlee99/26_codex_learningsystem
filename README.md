...
이 레포는 공개 포트폴리오용 코드베이스입니다. 개인 학습 DB, 실제 풀이 기록, Obsidian 산출물은 git에 올리지 않도록 제외했습니다. 포함된 SQLD 문항도 상업 문제집이나 실제 시험 덤프가 아니라, 시스템 동작을 보여주기 위한 합성 훈련 문항입니다.
...
| SQLD 문제 seed | `cert_study/seed_sqld.py` | 합성 SQLD 훈련 문항 50개 |
...
| 검증 항목 | 내용 |
| --- | --- |
| SQLD seed bank | 합성 SQLD 문항 50개 로드 |
...
| MCP 도구 | `start_session`, `submit_answer`, `finish_session`, `prepare_notion_sync` 도구 노출 |
...
이 공개 레포는 `SQLD`를 단일 기본 과목으로 둡니다.

과목 하나를 더 추가하는 정도라면 지금 seed 패턴을 복사하면 됩니다.
...
실제 과목이 2~3개 이상이 되면 Python seed 파일을 계속 늘리기보다 JSON/YAML 문제은행 가져오기 도구로 바꾸는 게 맞습니다. 개인 학습 목표 목록은 아래처럼 잡았습니다.
