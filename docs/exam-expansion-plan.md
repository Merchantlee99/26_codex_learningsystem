...
이 문항 수는 실전 학습용 충분량이 아니라 공개 데모용 1회분입니다. 실전 학습은 개인 문제은행을 로컬에서 키우고, 엔진이 미풀이/약점/복습 우선순위로 출제하는 구조를 전제로 합니다.
...
| 모드 | 우선순위 |
| --- | --- |
| `custom-cbt` | 미풀이 문제 -> 복습 예정 문제 -> 오답 이력 개념 -> 오래 전에 푼 문제 -> 최근에 푼 문제 |
| `review-cbt` | 복습 예정 오답 -> 오답 이력 문제 -> 미풀이 문제 -> 오래 전에 푼 문제 |
| `weak-cbt` | 자주 틀린 개념의 문항 -> 복습 예정 문제 -> 미풀이 문제 -> 오래 전에 푼 문제 |
