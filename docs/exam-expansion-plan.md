...
AWS Solutions Architect Associate는 SAA-C03 기준의 오픈 라이선스 Markdown 원천을 로컬에 둔 뒤 전용 변환기를 쓸 수 있습니다. 현재 엔진은 단일 선택형 CBT만 지원하므로 변환기는 단일 선택형 문항만 가져오고, 다중 선택형 문항은 제외합니다.
...
| `AWS_SOLUTIONS_ARCHITECT_ASSOCIATE` | AWS Certified Solutions Architect Associate | 클라우드 아키텍처 | 로컬 MIT Markdown 원천 inspector와 변환기 있음 |
